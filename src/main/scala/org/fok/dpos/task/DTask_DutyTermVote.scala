...
import org.fok.dpos.model.Dposblock.PSDutyTermVote.BlockRange
import org.fok.dpos.model.Dposblock.PSDutyTermVote.TermBlock
...
import org.fok.dpos.model.Dposblock.PSDutyTermVote.RewriteTerm
  ...
  def checkPossibleTerm(vq: PSDutyTermVote.Builder)(implicit network: Network): Boolean = {
    ...
    possibleTermID.map(f => {
      ...
      if (DCtrl.coMinerByUID.containsKey(bcuid)) {
        val records = Daos.dposvotedb.listBySecondKey(("D" + f._1).getBytes);

        val reclist: Buffer[PDutyTermResult.Builder] = records.get.values().map { p =>
          PDutyTermResult.newBuilder().mergeFrom(p);
        }.toBuffer;
